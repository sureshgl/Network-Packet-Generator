...
public abstract class AbstractBaseExt implements GetFormattedText{
	...
	protected void getCases(List<Pair<String, String>> cases, StatesInfo statesPathGenerator) {
		...
		if(ctx != null && ctx.children != null && ctx.children.size()>0){
			for(ParseTree childCtx : ctx.children){
				if(!(childCtx instanceof TerminalNode)){
					getExtendedContext(childCtx).getCases(cases,statesPathGenerator);
				}
			}
		}
	}
	...
	protected void getFields(List<Field> fields,SymbolTable st){
		...
		if(ctx != null && ctx.children != null && ctx.children.size()>0){
			for(ParseTree childCtx : ctx.children){
				if(!(childCtx instanceof TerminalNode)){
					getExtendedContext(childCtx).getFields(fields,st);
				}
			}
		}
	}
	...
	protected String getParserBlockName(){
		ParserRuleContext ctx = getContext();
		if(ctx != null && ctx.children != null && ctx.children.size()>0){
			for(ParseTree childCtx : ctx.children){
				if(!(childCtx instanceof TerminalNode)){
					String out = getExtendedContext(childCtx).getParserBlockName();
					if(out != null){
						return out;
					}
				}
	...
	public void populateErrors(List<String> errors){
		...
		if(ctx != null && ctx.children != null && ctx.children.size()>0){
			for(ParseTree childCtx : ctx.children){
				if(!(childCtx instanceof TerminalNode)){
					getExtendedContext(childCtx).populateErrors(errors);
				}
			}
		}
	}
	
	//to populate all the string template fields
	public void populateSTG(List<ST> stmts,STGroupFile grp, StatesInfo spg, SymbolTable st) {
		ParserRuleContext ctx = getContext();
		if(ctx != null && ctx.children != null && ctx.children.size()>0){
			for(ParseTree childCtx : ctx.children){
				if(!(childCtx instanceof TerminalNode)){
					getExtendedContext(childCtx).populateSTG(stmts, grp, spg, st);
				}
			}
		}
	}

	//for parsing parse state
	public void populateParseSTG(List<ST> stmts,Map<String, String> fields, STGroupFile grp, StatesInfo spg, SymbolTable st) {
		ParserRuleContext ctx = getContext();
		if(ctx != null && ctx.children != null && ctx.children.size()>0){
			for(ParseTree childCtx : ctx.children){
				if(!(childCtx instanceof TerminalNode)){
					getExtendedContext(childCtx).populateParseSTG(stmts, fields, grp, spg, st);
				}
			}
		}
	}
